    ...
    function draw() {
      ...
      particles.forEach((p,i) => particles.slice(i+1).forEach(q => {
        ...
        if(d<90){ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.lineTo(q.x,q.y)
          ctx.strokeStyle=`rgba(108,95,244,${.07*(1-d/90)})`;ctx.lineWidth=.5;ctx.stroke()}
      }))
...
function MouseGlow() {
  ...
  return <motion.div style={{left:sx,top:sy,x:'-50%',y:'-50%',position:'fixed',pointerEvents:'none',zIndex:1,width:500,height:500,borderRadius:'50%',background:'radial-gradient(circle,rgba(108,95,244,.1) 0%,transparent 70%)',filter:'blur(2px)'}} />
}
...
function FeatureCard({icon,title,desc,delay}) {
  return(
    <motion.div initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} transition={{delay,duration:.6,ease:[.16,1,.3,1]}}
      style={{display:'flex',gap:14,padding:'14px 16px',borderRadius:12,background:'rgba(255,255,255,.03)',border:'1px solid rgba(255,255,255,.06)',backdropFilter:'blur(10px)'}}>
      <div style={{width:34,height:34,borderRadius:9,background:'rgba(108,95,244,.15)',border:'1px solid rgba(108,95,244,.25)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0,color:'#a29afb'}}>{icon}</div>
      <div>
        <div style={{fontSize:13,fontWeight:600,color:'rgba(255,255,255,.85)',marginBottom:2}}>{title}</div>
...
function AuthForm() {
  ...
  const inp=(f)=>({
    ...
    boxShadow:focused===f?'0 0 0 3px rgba(108,95,244,.12),0 0 20px rgba(108,95,244,.08)':'none'
  })
  ...
  const css=`
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    ::-webkit-scrollbar-thumb{background:rgba(108,95,244,.3);border-radius:2px}
  `
  ...
  return(
    <div style={{minHeight:'100vh',background:'#03030a',display:'flex',overflow:'hidden',position:'relative',fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{position:'fixed',top:'35%',right:'25%',width:'25vw',height:'25vw',borderRadius:'50%',background:'radial-gradient(circle,rgba(167,139,250,.07) 0%,transparent 65%)',pointerEvents:'none',zIndex:0,filter:'blur(60px)'}}/>
      ...
      <div className="left-side" style={{flex:1,display:'flex',flexDirection:'column',justifyContent:'center',padding:'60px 4% 60px 7%',position:'relative',zIndex:2}}>
        ...
        <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:.1}}
          style={{display:'inline-flex',alignItems:'center',gap:6,padding:'5px 12px',borderRadius:9999,background:'rgba(108,95,244,.1)',border:'1px solid rgba(108,95,244,.25)',fontSize:11,fontWeight:600,color:'#a29afb',letterSpacing:'.06em',textTransform:'uppercase',marginBottom:18,width:'fit-content'}}>
          <span style={{width:5,height:5,borderRadius:'50%',background:'#34d399',display:'inline-block'}}/>
          AI-Powered Identity
        </motion.div>

        <motion.h1 initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{delay:.15,duration:.7,ease:[.16,1,.3,1]}}
          style={{fontFamily:"'Syne',sans-serif",fontSize:'clamp(34px,3.8vw,54px)',fontWeight:800,letterSpacing:'-.04em',lineHeight:1.08,marginBottom:18,background:'linear-gradient(160deg,#fff 30%,rgba(162,154,251,.85) 65%,rgba(56,189,248,.7) 100%)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text'}}>
          Create your<br/>digital universe
        </motion.h1>

        <motion.p initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:.25,duration:.6}}
          style={{fontSize:15,color:'rgba(255,255,255,.42)',lineHeight:1.8,maxWidth:360,marginBottom:32}}>
        ...
        <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:.7}} style={{marginTop:32,display:'flex',alignItems:'center',gap:12}}>
          <div style={{display:'flex'}}>
            {['#6c5ff4','#38bdf8','#34d399','#f472b6','#fbbf24'].map((c,i)=>(
              <div key={i} style={{width:26,height:26,borderRadius:'50%',background:`linear-gradient(135deg,${c},${c}88)`,border:'2px solid #03030a',marginLeft:i?-8:0,display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,color:'#fff',fontWeight:700}}>
                {['A','K','V','P','R'][i]}
              </div>
            ))}
          </div>
      ...
      <div style={{width:'min(480px,100%)',display:'flex',alignItems:'center',justifyContent:'center',padding:'40px 28px',position:'relative',zIndex:2}}>
        <motion.div initial={{opacity:0,y:32,scale:.96}} animate={{opacity:1,y:0,scale:1}} transition={{duration:.7,ease:[.16,1,.3,1]}}
          ...
          <div style={{display:'flex',background:'rgba(255,255,255,.04)',borderRadius:11,padding:3,marginBottom:24,border:'1px solid rgba(255,255,255,.06)'}}>
            {['login','register'].map(m=>(
              <button key={m} onClick={()=>{setMode(m);setError(null)}}
                style={{flex:1,padding:'9px',borderRadius:9,fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:'inherit',border:'none',transition:'all .2s',
                  background:mode===m?'linear-gradient(135deg,rgba(108,95,244,.85),rgba(108,95,244,.55))':'transparent',
                  color:mode===m?'#fff':'rgba(255,255,255,.35)',
                  boxShadow:mode===m?'0 2px 12px rgba(108,95,244,.3)':'none'}}>
          ...
          {/* Google */}
          <motion.button whileHover={{scale:1.02,background:'rgba(255,255,255,.09)'}} whileTap={{scale:.98}}
            onClick={handleGoogle} disabled={loading}
            style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:10,padding:'11px',borderRadius:11,fontSize:13,fontWeight:500,marginBottom:18,cursor:'pointer',fontFamily:'inherit',background:'rgba(255,255,255,.06)',border:'1px solid rgba(255,255,255,.1)',color:'#fff',transition:'all .2s'}}>
          ...
          <AnimatePresence>
            {error&&(
              <motion.div initial={{opacity:0,y:-8,height:0}} animate={{opacity:1,y:0,height:'auto'}} exit={{opacity:0,height:0}}
                style={{display:'flex',alignItems:'center',gap:8,padding:'10px 12px',borderRadius:9,marginBottom:14,fontSize:12,background:'rgba(248,113,113,.08)',border:'1px solid rgba(248,113,113,.2)',color:'#fca5a5'}}>
          ...
          <form onSubmit={handleSubmit} style={{display:'flex',flexDirection:'column',gap:12}}>
              ...
              {mode==='register'&&(
                <motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:'auto'}} exit={{opacity:0,height:0}} transition={{duration:.22}}>
                  ...
                  <div style={{position:'relative'}}>
                    ...
                    <input value={name} onChange={e=>setName(e.target.value)} placeholder="Arjun Mehta" required
            ...
            {mode==='register'&&(
              <p style={{fontSize:11,color:'rgba(255,255,255,.28)',lineHeight:1.6}}>
                By registering you agree to our{' '}
                <Link href="/legal/terms" style={{color:'#a29afb',textDecoration:'none'}}>Terms</Link> &amp;{' '}
                <Link href="/legal/privacy" style={{color:'#a29afb',textDecoration:'none'}}>Privacy Policy</Link>.
              </p>
            )}

            <motion.button type="submit" disabled={loading}
              whileHover={!loading?{scale:1.02,boxShadow:'0 0 40px rgba(108,95,244,.6)'}:{}}
